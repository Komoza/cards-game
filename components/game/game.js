    ...
    cards.forEach((card) => {
        card.addEventListener('click', () => {
            ...
            if (card.dataset.status !== 'open') {
                card.setAttribute(
                    'src',
                    `./assets/image/card/${card.dataset.value}.jpg`
                );
...
const fillCardsArray = (numbers) => {
    ...
    for (let i = 0; i < numbers / 2; i++) {
        ...
        do {
            ...
            arrCards.includes(
                `<img data-value="${rank}-of-${suit}" class="game__card" src="./assets/image/card/${rank}-of-${suit}.jpg" alt="card"/>`
            )
        );

        let card = `<img data-value="${rank}-of-${suit}" class="game__card" src="./assets/image/card/${rank}-of-${suit}.jpg" alt="card"/>`;
        arrCards.push(card);
    ...
    setTimeout(() => {
        const cards = document.querySelectorAll('.game__card');
        cards.forEach((card) => {
            card.setAttribute('src', './assets/image/card/dealer-shirt.jpg');
        });
    }, 5000);
};
